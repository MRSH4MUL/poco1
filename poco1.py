#---------------------------------------------
#-------SEND :- https://t.me/ZERO_CYBER_TEM
#ZERO TEM
#---------------------------------------------
...
def confirm_id(mail,uid,otp,data,ses):
    try:
        ...
        if "checkpoint" in str(response.url):
            ...
            print(f'''\r{xd}-{W}[{G}MRPOCO-OK{W}]{G} {uid} | KALYAN3727 ''')
            print(f'''\r{xd}-{cookie} ''');linex()
            open('/sdcard/MRPOCO-AUTO-CRATE-OK-ID.txt', 'a').write(uid + '|KALYAN3727|' + cookie + '\n')
            oks.append(uid)
    except Exception as e:
        pass
